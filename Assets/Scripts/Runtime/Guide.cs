        ...
        public void Show(HeartCondition condition)
        {
            ...
            switch (condition)
            {
                ...
                case HeartCondition.Tachycardia:
                    ...
                    _descripton.text = $"A elevated heart rate above 190 BPM. The condition can be caused by the following:\n •\tDiabetes, high blood suger levels greater than {_settings.bloodSugarDangerousHigh} mg/dL. \n •\tPre-existing heart ccondition.\n\nBeing Tachycardic for too long will result in Cardiac Arrest or Atrial Fibrillation. See the respective entries for more infomation about treating these conditions.";
                    _traetment.text = $"\n• Heart rate can be lowered with Beta-Blockers.\n\n• If blood suger is high treat with insulin.";
                    break;
                case HeartCondition.Bradycardia:
                    ...
                    _descripton.text = $"A slow heart rate below 22 BPM. The condition can be caused by the following:\n •\tStarving, low blood suger levels less than {_settings.bloodSugarDangerousLow} mg/dL. \n •\tLow Oxygen (SpO2) levels less then {_settings.oxygenDangerousLevel}%.\n •\tPre-existing heart ccondition.\n\nBeing Bradycardic for too long will result in Cardiac Arrest or a Heat Block. See furture entries for more infomation about treating these conditions.";
                ...
                case HeartCondition.VentricularFibrillation:
                    ...
                    _descripton.text = $"A contition that causes the heart to contract in a abnormal heart rhythm. This looks like random noise on the EKG. This condition immediately cause a Cardiac Arrest.";
                    _traetment.text = $"\n• Patients with Ventricular Fibrillationcan must be reivived with a defibrillator and then treated with then with Furosemide to prevent further eposides.";
                    break;
                case HeartCondition.Hypothermia:
                    _heart.SetJWave(1f);

                    _descripton.text = $"The patients body temperture is lower than 30 degrees. Hypothermia causes an adnormal EKG which can be seen as a second peak (J-Wave). This condition can cause a Cardiac Arrest if left untreated.";
                    _traetment.text = $"\n• Patients with Hypothermia can be treated by rasing their room temperture.";
                    break;
                case HeartCondition.CardiacArrest:
                    ...
                    _traetment.text = $"\n• Patients with Cardiac Arrest can be revived with a defibrillator. The condition that caused the Cardiac Arrest must be treat to prevent further eposides.";
                    break;
